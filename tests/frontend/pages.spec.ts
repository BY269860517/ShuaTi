...
describe('pdf import frontend pages', () => {
  ...
  test('wrong page lists active wrong questions and starts wrong practice', () => {
    ...
    expect(source).toContain('header__rule')
    expect(source).toContain('答错会自动加入错题本')
    expect(source).toContain('连续答对 3 次视为已掌握并移出当前列表')
    expect(source).toContain('再次答错会重新加入')
  ...
  test('candidate review page groups candidates and confirms import', () => {
    ...
    const confirmImportStart = source.indexOf('async function confirmImport')
    const confirmImportEnd = source.indexOf('function showRemoveConfirm')
    const confirmImportSource = source.slice(confirmImportStart, confirmImportEnd)
    const removeHandlerStart = source.indexOf('async function confirmRemoveCandidate')
    const removeHandlerEnd = source.indexOf('function editCandidate')
    const removeHandlerSource = source.slice(removeHandlerStart, removeHandlerEnd)
    const buttonStart = source.indexOf('class="bottom-actions__button"')
    const buttonEnd = source.indexOf('@click="confirmImport"', buttonStart)
    ...
    expect(source).toContain('api.candidateDelete(candidateId)')
    expect(source).toContain('@remove="confirmRemoveCandidate"')
    expect(source).toContain('candidates.value = candidates.value.filter')
    expect(source).toContain('uni.showModal')
    expect(source).toContain('移除此候选题')
    expect(source).toContain('safe-area-inset-bottom')
    expect(source).toContain("const removeConfirmingId = ref('')")
    expect(confirmImportSource).toContain('removingId.value')
    expect(confirmImportSource).toContain('removeConfirmingId.value')
    expect(removeHandlerSource).toContain('removeConfirmingId.value = candidateId')
    expect(removeHandlerSource).toContain('removeConfirmingId.value = \'\'')
    expect(removeHandlerSource).toContain('if (removingId.value || confirming.value) return')
    expect(buttonSource).toContain('removingId')
    expect(buttonSource).toContain('removeConfirmingId')
  })

  test('candidate cards expose remove for non-imported and non-importing candidates', () => {
    const source = read('components/CandidateCard.vue')
    const headerStart = source.indexOf('.candidate-card__header')
    const headerEnd = source.indexOf('.candidate-card__heading')
    const headerSource = source.slice(headerStart, headerEnd)
    const headingStart = source.indexOf('.candidate-card__heading')
    const headingEnd = source.indexOf('.candidate-card__number')
    const headingSource = source.slice(headingStart, headingEnd)
    const actionsStart = source.indexOf('.candidate-card__actions')
    const actionsEnd = source.indexOf('.candidate-card__edit')
    ...
    expect(source).toContain("props.candidate.status !== 'imported' && props.candidate.status !== 'importing'")
    expect(source).toContain("props.candidate.status !== 'imported'")
    expect(source).toContain("props.candidate.status !== 'importing'")
    expect(source).toContain('v-if="canEdit"')
    expect(source).toContain("emit('remove', props.candidate._id)")
    expect(source).toContain('移除')
    expect(source).toContain("v-else-if=\"props.candidate.status === 'importing'\"")
    expect(source).toContain('导入中')
    expect(headerSource).toContain('flex-wrap: wrap')
    expect(headingSource).toContain('min-width: 240rpx')
    expect(actionsSource).toContain('flex-shrink: 0')
  })
  ...
  test('practice setup loads question count, offers enhanced choices, and creates sessions', () => {
    ...
    expect(source).toContain("from '@/common/practiceSettings'")
    expect(source).toContain('normalizeCustomCountInput')
    expect(source).toContain('resolvePracticeCount')
    expect(source).toContain('getPracticeCreateErrorMessage')
    expect(source).toContain('api.practiceCreate({')
    expect(source).toContain('count: selectedPracticeCount.value')
    ...
    expect(source).toContain("countMode = ref<PracticeCountMode>('fixed')")
    expect(source).toContain("orderMode = ref<PracticeOrderMode>('sequence')")
    expect(source).toContain("practiceScope = ref<PracticeScope>('all')")
    expect(source).toContain("questionType = ref<PracticeQuestionTypeFilter>('all')")
    expect(source).toContain('selectedPracticeCount')
    expect(source).toContain('customCountInput')
    expect(source).toContain('sanitizeCustomCountInput')
    expect(source).not.toContain('replace(/\\D/g')
    expect(source).toContain('filteredQuestionCount')
    expect(source).toContain('canCreatePractice')
    expect(source).toContain('setupHint')
    expect(source).toContain('未练习数量会在创建时由云端确认')
    ...
    expect(source).toContain('return `${scopeLabel}，${orderLabel}出题，系统会按当前设置创建练习。`')
    expect(source).toContain(':disabled="!canCreatePractice"')
    expect(source).toContain('uni.redirectTo({ url: `/pages/practice/do?sessionId=${result.session._id}` })')
